...
void	ft_bfile(t_all *s, int fd)
{
	...
	n = s->win.x * s->win.y * 4 + 54; // 헤더파일크기 14 + 40, 가로*세로*픽셀당 파일크기(4)
...
// 2바이트, BMP 파일 매직 넘버. 비트맵 파일이 맞는지 확인하는데 사용하며 ASCII 코드로 0x42(B), 0x4D(M)가 저장됩니다.
// 
...
int		ft_bitmap(t_all *s)
{
	...
	ft_bdraw(s);  //ray, hit 초기화
	fd = open("saved_bmp.bmp", O_CREAT | O_RDWR); //O_RDWR : 읽고쓰기 가능, O_CREAT: 파일이 없으면 생성 
	ft_bfile(s, fd);
	ft_binfo(s, fd);
	ft_bdata(s, fd);
	close(fd);
