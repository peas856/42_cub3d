...
void	ft_screen(t_all *s)
{
	...
	s->img.ptr = mlx_new_image(s->mlx.ptr, s->win.x, s->win.y);  //mlx_new_image()로 img식별자(void*)를 받고
	s->img.adr = (unsigned int *)mlx_get_data_addr(s->img.ptr, &bpp, &sl, &end);  //mlx_get_data_addr()로 img데이터(char*)를 받아서
